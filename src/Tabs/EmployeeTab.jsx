...
const EmployeeTab = () => {
  ...
  const employeeTab = [
    { label: 'Create Employee', value: '/employee/create' },
    { label: 'All Employee', value: '/employee/list' },
  ];

  // Get current route to set the active tab
  const currentPath = location.pathname;
  const value =
    employeeTab.find((tab) => currentPath.startsWith(tab.value))?.value || '/users/list';

  // Get the label for the current tab or default to a fallback
  const currentTab = employeeTab.find((tab) => tab.value === value);
