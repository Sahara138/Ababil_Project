...
const UpdateHajjVoucher = () => {
  const theme = useTheme();
  const isDarkMode = theme.palette.mode === "dark";
  const navigate = useNavigate();
  const { id } = useParams(); // Assuming the voucher ID is passed as a route parameter
  ...
  // Fetch the existing voucher details
  useEffect(() => {
    ...
    if (validate()) {
      ...
      fetch(`http://localhost:8000/vouchers/${id}`, {
        ...
        .then((res) => {
          ...
          navigate("/hajj/vouchers"); // Redirect after successful update
        })
        .catch((err) => console.log(err.message));
    } else {
...
export default UpdateHajjVoucher;
