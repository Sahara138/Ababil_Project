...
const UpdateHajjPayment = () => {
  const theme = useTheme();
  const isDarkMode = theme.palette.mode === 'dark';
  const navigate = useNavigate();
  const { id } = useParams(); // Retrieve the payment ID from the URL
  ...
  // Fetch payment data when the component loads
  useEffect(() => {
    fetch(`http://localhost:8000/payments/${id}`)
      .then((res) => res.json())
      .then((data) => {
        setPaymentId(data.paymentId || "");
        setPilgrimName(data.pilgrimName || "");
    ...
    if (validate()) {
      fetch(`http://localhost:8000/payments/${id}`, {
        ...
        .then((res) => {
          ...
          navigate("/hajj/payments"); // Redirect after successful update
        })
        .catch((err) => console.log(err.message));
    }
  };
  ...
  return (
    <div style={{ width: '100%' }}>
      ...
      >
        ...
        <div className="payment-update" style={{ width: "100%" }}>
...
export default UpdateHajjPayment;
